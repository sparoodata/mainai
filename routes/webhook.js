...
/*
  Helper: Forward slash commands (e.g., "/hi") to the Groq endpoint.
  Sends a POST request with a JSON payload containing phoneNumber and message.
*/
async function forwardToGroq(phoneNumber, message) {
  try {
    ...
    console.error('Error forwarding to Groq:', error.response ? error.response.data : error.message);
...
const interactive = messageObj?.interactive || null;
  ...
  if (body.object === 'whatsapp_business_account') {
    ...
    if (value.messages) {
      ...
      // NEW: If the message starts with '/', forward it to Groq and send its reply.
