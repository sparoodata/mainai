...
module.exports = (groq) => {
  ...
  async function getGroqAIResponse(message, phoneNumber) {
    try {
      const systemPrompt = "You are an AI assistant for a rental management app. Provide helpful responses to user queries.";
  ...
  // Helper function to send the default "Use 'Help'" message with icons
  async function sendHelpPrompt(phoneNumber) {
    const helpMessage = `Use 'Help' for menu options:\n\n` +
      ...
      `🏠 *Manage* - Manage properties, units, or tenants\n` +
      `💸 *Transactions* - Handle rent payments`;
...
- Verified Date: ${user.verifiedDate ? user.verifiedDate.toLocaleString() : 'N/A'}
  ...
  async function sendPropertyOptions(phoneNumber) {
    try {
      ...
      if (!user) {
        await sendMessage(phoneNumber, 'User not found. Please register first.');
        return;
      }
      ...
      if (!hasProperties) {
        await sendMessage(phoneNumber, 'No properties found to edit or remove. Please create a property first.');
      }

      const buttons = [];
      buttons.push({ type: 'reply', reply: { id: 'add_property', title: 'Add Property' } });
      if (hasProperties) {
        buttons.push({ type: 'reply', reply: { id: 'edit_property', title: 'Edit Property' } });
        buttons.push({ type: 'reply', reply: { id: 'remove_property', title: 'Remove Property' } });
      }
      ...
      console.error('Error in sendPropertyOptions:', error);
      await sendMessage(phoneNumber, 'An error occurred. Please try again.');
    }
  }
  ...
  async function sendUnitOptions(phoneNumber) {
    try {
      ...
      if (!hasProperties) {
        await sendMessage(phoneNumber, 'No properties found. Please create a property before adding units.');
        return;
      }

      if (!hasUnits) {
        await sendMessage(phoneNumber, 'No units found to edit or remove. Please create a unit first.');
      }

      const buttons = [];
      buttons.push({ type: 'reply', reply: { id: 'add_unit', title: 'Add Unit' } });
      if (hasUnits) {
        buttons.push({ type: 'reply', reply: { id: 'edit_unit', title: 'Edit Unit' } });
        buttons.push({ type: 'reply', reply: { id: 'remove_unit', title: 'Remove Unit' } });
      }
      ...
      console.error('Error in sendUnitOptions:', error);
      await sendMessage(phoneNumber, 'An error occurred. Please try again.');
    }
  }
  ...
  async function sendTenantOptions(phoneNumber) {
    try {
      ...
      if (!hasUnits) {
        await sendMessage(phoneNumber, 'No units found. Please create a unit before adding tenants.');
        return;
      }

      if (!hasTenants) {
        await sendMessage(phoneNumber, 'No tenants found to edit or remove. Please create a tenant first.');
      }

      const buttons = [];
      buttons.push({ type: 'reply', reply: { id: 'add_tenant', title: 'Add Tenant' } });
      if (hasTenants) {
        buttons.push({ type: 'reply', reply: { id: 'edit_tenant', title: 'Edit Tenant' } });
        buttons.push({ type: 'reply', reply: { id: 'remove_tenant', title: 'Remove Tenant' } });
      }
      ...
      console.error('Error in sendTenantOptions:', error);
      await sendMessage(phoneNumber, 'An error occurred. Please try again.');
    }
  }
  ...
  console.log('Webhook exports:', exports);
  return exports;
};