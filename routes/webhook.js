...
// Helper: Check if a string contains only numbers.
...
/*
  Send an interactive list for country selection.
  Enterprise version: Only "India" is allowed.
*/
...
/*
  Send interactive buttons for property type selection.
*/
...
async function sendImageOptionButton(fromNumber, type, entityId) {
  ...
  const messageData = {
    ...
    interactive: {
      ...
      body: { text: 'If you prefer to skip image upload, tap below:' },
      action: {
        buttons: [
  ...
  if (body.object === 'whatsapp_business_account') {
    ...
    if (value.messages) {
      ...
      if (text) {
        ...
        if (text.toLowerCase() === 'help') {
          const buttonMenu = {
            ...
            interactive: {
              ...
              body: { text: 'Welcome to the Enterprise Rental Management System. Please choose an option:' },
              footer: { text: '© Enterprise Rental Management' },
              action: {
                buttons: [
        ...
        // Enterprise-style property creation flow (example for property name, description, etc.)
        if (sessions[fromNumber].action === 'add_property_name') {
          if (isValidName(text)) {
            sessions[fromNumber].propertyData = { name: text };
            await sendMessage(fromNumber, 'Please provide a concise description for the property (max 100 characters).');
          ...
          if (text.length > 100) {
            await sendMessage(fromNumber, 'Description exceeds 100 characters. Please re-enter a shorter description.');
            return res.sendStatus(200);
          }
          sessions[fromNumber].propertyData.description = text;
          await sendMessage(fromNumber, 'Enter the street address of the property:');
          ...
          if (isValidAddress(text)) {
            ...
            await sendMessage(fromNumber, 'Enter the city where the property is located:');
          ...
          if (isValidUnits(text)) {
            ...
            await sendMessage(fromNumber, 'Invalid number of units. Please re-enter.');
          }
        ...
        // --- Add-Unit and Tenant Flows (similar enterprise-style messaging) ---
        // [For brevity, these flows remain similar to your previous implementation,
        // with adjusted language if desired.]
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`
            : 'No account information available for this number.';
        ...
        // (Additional interactive reply processing for property/unit/tenant creation)
        // ...
        // --- Image Upload Choice ---
          ...
          if (selectedOption.startsWith('upload_')) {
            ...
            const shortUrl = await shortenUrl(imageUploadUrl);
            await sendMessage(fromNumber, `Please upload the image using this secure link (valid for 15 minutes): ${shortUrl}`);
            sessions[fromNumber] = {};
...
// Summary function: Sends a single message (image with caption) using only the first image.
